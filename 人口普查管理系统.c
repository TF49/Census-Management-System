...
void register_user()
{
	...
	FILE* file;
	errno_t err = fopen_s(&file, USERS_FILE, "a+");// 以追加模式打开文件，如果文件不存在则创建
	...
	// 写入文件
	fprintf(file, "%s %s\n", input_username, input_password);//将用户名和密码写入文件，每行存储一对用户名和密码，并用空格分隔
	fclose(file);
	printf("注册成功!\n");
	system("pause"); // 暂停
	system("cls");   // 清屏
...
int login()
{
	...
	FILE* file;
	errno_t err = fopen_s(&file, USERS_FILE, "r"); // 打开 USERS_FILE 文件读取所有已注册的用户信息。
	if (err != 0 || !file) {
		printf("无法打开文件!\n");
		return 0;
	}
	...
	while (fgets(line, sizeof(line), file)) { // 调用user.txt中的相关数据 
		sscanf_s(line, "%s %s", stored_username, sizeof(stored_username), stored_password, sizeof(stored_password)); // 解析文本，将用户名和密码分别存入 stored_username 和 stored_password 变量中,对每一行数据使用 sscanf_s 解析出用户名和密码
